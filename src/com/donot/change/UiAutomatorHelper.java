...
public class UiAutomatorHelper {
	...
	/**
	 * 需求：UI工程调试构造器，输入jar包名，包名，类名，用例名，项目路径（mac intellij idea 获取项目目录失败）
	 */
	public UiAutomatorHelper(String jarName, String testClass, String testName,
							 String androidId, String worksp,int i) {
		System.out.println("-----------start--uiautomator--debug-------------");
		workspace_path = worksp;
		System.out.println("----工作空间：\t\n" + worksp);
	...
	public void modfileBuild() {
		...
		try {
			System.out.print("workspace_path: " + workspace_path);
			File file = new File(workspace_path+"//"+"build.xml");
			if (file.isFile() && file.exists()) { // 判断文件是否存在
				InputStreamReader read = new InputStreamReader(
						new FileInputStream(file));
				BufferedReader bufferedReader = new BufferedReader(read);
				System.out.print("文件是存在的！！！");
				String lineTxt = null;
	...
	public void buildWithAnt() {
		if (System.getProperty("os.name").contains("Mac")) {
			execCmd("/Users/apple/autoTest/apache-ant-1.10.1/bin/ant build");
			return;
		}
		execCmd("cmd /c ant");
	}
...
//		System.out.print("abPath"+ abPath);
	...
	public void writerText(String path, String content) {
		...
		System.out.print("write path: " + workspace_path+"/"+path);
		try {
			// new FileWriter(path + "t.txt", true) 这里加入true 可以不覆盖原有TXT文件内容 续写
			BufferedWriter bw1 = new BufferedWriter(new FileWriter(workspace_path+"/"+path));
			bw1.write(content);
			bw1.flush();
			bw1.close();
		} catch (IOException e) {
